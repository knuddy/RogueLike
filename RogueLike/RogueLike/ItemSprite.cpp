...
void ItemSprite::ApplyEffects(PlayerSprite ^ player)
{
	...
	case HEALTH:
		...
		{
			int healthRecovered = Constants::HEALTH_RESTORE_VALUE - calculateHealthIncrease(player);
			player->IncreaseHealth(); //Restore HP of the player
			messageController->Message = "Health has been\nrestored by " + healthRecovered.ToString(); +"!";
			IsAlive = false;
		}
...
int ItemSprite::calculateHealthIncrease(PlayerSprite ^ player)
{

	int healthIncrease = Constants::HEALTH_RESTORE_VALUE - (player->MaxHealth - player->Health);

	if (healthIncrease > 0) //Only want numbers that are less that the max restore value
	{
